...
int	ft_pf_tools_d2s(t_printf *f, unsigned int n, char *str)
{
	...
	i = 0;
	base = ft_pf_base2radix((*f).at);
	while (i < (*f).p_num)
	{
		str[i++] = ft_itoc_base((char)(n % base), (*f).at);
		n = n / base;
	}
	return (i);
}
...
int	ft_pf_tools_i2s(char arg_type, unsigned int n, char *str)
{
	...
	i = 0;
	base = ft_pf_base2radix(arg_type);
	if (!n)
		str[i++] = '0';
	while (n)
	{
		str[i++] = ft_itoc_base((char)(n % base), arg_type);
		n = n / base;
	}
	return (i);
}
...
int	ft_pf_f2s(t_printf *f, char *str)
{
	...
	ft_pf_fwp_u(f, &arg);
	i = ft_pf_f2s_power(f, arg_arr, str, &arg);
	if ((*f).at == 'f')
...
/// @return		the number pf chars of the result
int	ft_pf_f2s_power(t_printf *f, char *arg_arr, char *str, void *arg)
{
	int		i;
	char	base;
